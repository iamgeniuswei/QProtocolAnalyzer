...
static int ett_frame = -1;
static int ett_flags = -1;
static int ett_comments = -1;

//static QPAExpertInfo ei_comments_text;
//static QPAExpertInfo ei_arrive_time_out_of_range;
//static QPAExpertInfo ei_incomplete;
...
static bool generate_epoch_time = true;
...
static int
dissect_frame(shared_ptr<QPATvbuff> tvb, shared_ptr<QPAPacketInfo> pinfo, shared_ptr<QPAProtoNode> parent_tree, void* data)
{
        QPAProtoNode* ti = nullptr;
        unsigned int cap_len = tvb->length();
        unsigned int frame_len = tvb->reported_length();
        shared_ptr<QPAProtoNode> tree = parent_tree, comments_tree = nullptr, item = nullptr;
        string cap_plurality = plurality(cap_len, "", "s"), frame_plurality = plurality(frame_len, "", "s");
        
        
        switch(pinfo->_phdr->rec_type())
        {
            case REC_TYPE_PACKET:
                pinfo->_current_proto = "Frame";
                break;
                
            case REC_TYPE_FT_SPECIFIC_EVENT:
                pinfo->_current_proto = "Event";
                break;
                
            case REC_TYPE_FT_SPECIFIC_REPORT:
                pinfo->_current_proto = "Report";
                break;
        ...
        ti = tree->proto_tree_add_protocol_format(proto_frame, tvb, 0, tvb->length(),
                "Frame %u: %u byte%s on wire",
                                                  1, frame_len, frame_plurality.c_str());
        cout << ti->finfo->rep << endl;
...
proto_register_frame(void)
{
    ...
    QPAHeaderFieldRegisterInfo hf[] = {
        {
            ...
            make_shared<QPAHeaderFieldInfo>("Arrival Time", "frame.time",
                                            FT_ABSOLUTE_TIME, ABSOLUTE_TIME_LOCAL, nullptr, 0x0,
                                            "Absolute time when this frame was captured")
        },
        { hf_frame_shift_offset,
            make_shared<QPAHeaderFieldInfo>("Time shift for this packet", "frame.offset_shift",
                                            FT_RELATIVE_TIME, BASE_NONE, nullptr, 0x0,
                                            "Time shift applied to this packet")},
        { hf_frame_arrival_time_epoch,
            make_shared<QPAHeaderFieldInfo>("Epoch Time", "frame.time_epoch",
                                            FT_RELATIVE_TIME, BASE_NONE, nullptr, 0x0,
                                            "Epoch time when this frame was captured")},

        { hf_frame_time_delta,
            make_shared<QPAHeaderFieldInfo>("Time delta from previous captured frame", "frame.time_delta",
                                            FT_RELATIVE_TIME, BASE_NONE, nullptr, 0x0,
                                            "")},

        { hf_frame_time_delta_displayed,
            make_shared<QPAHeaderFieldInfo>("Time delta from previous displayed frame", "frame.time_delta_displayed",
                                            FT_RELATIVE_TIME, BASE_NONE, nullptr, 0x0,
                                            "")},

        { hf_frame_time_relative,
            make_shared<QPAHeaderFieldInfo>("Time since reference or first frame", "frame.time_relative",
                                            FT_RELATIVE_TIME, BASE_NONE, nullptr, 0x0,
                                            "Time relative to time reference or first frame")},

        { hf_frame_time_reference,
            make_shared<QPAHeaderFieldInfo>("This is a Time Reference frame", "frame.ref_time",
                                            FT_NONE, BASE_NONE, nullptr, 0x0,
                                            "This frame is a Time Reference frame")},

        { hf_frame_number,
            make_shared<QPAHeaderFieldInfo>("Frame Number", "frame.number",
                                            FT_UINT32, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_frame_len,
            make_shared<QPAHeaderFieldInfo>("Frame length on the wire", "frame.len",
                                            FT_UINT32, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_frame_capture_len,
            make_shared<QPAHeaderFieldInfo>("Frame length stored into the capture file", "frame.cap_len",
                                            FT_UINT32, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_frame_md5_hash,
            make_shared<QPAHeaderFieldInfo>("Frame MD5 Hash", "frame.md5_hash",
                                            FT_STRING, BASE_NONE, nullptr, 0x0,
                                            "")},

        { hf_frame_p2p_dir,
            make_shared<QPAHeaderFieldInfo>("Point-to-Point Direction", "frame.p2p_dir",
                                            FT_INT8, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_link_number,
            make_shared<QPAHeaderFieldInfo>("Link Number", "frame.link_nr",
                                            FT_UINT16, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_frame_file_off,
            make_shared<QPAHeaderFieldInfo>("File Offset", "frame.file_off",
                                            FT_INT64, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_frame_marked,
            make_shared<QPAHeaderFieldInfo>("Frame is marked", "frame.marked",
                                            FT_BOOLEAN, BASE_NONE, nullptr, 0x0,
                                            "Frame is marked in the GUI")},

        { hf_frame_ignored,
            make_shared<QPAHeaderFieldInfo>("Frame is ignored", "frame.ignored",
                                            FT_BOOLEAN, BASE_NONE, nullptr, 0x0,
                                            "Frame is ignored by the dissectors")},

        { hf_frame_protocols,
            make_shared<QPAHeaderFieldInfo>("Protocols in frame", "frame.protocols",
                                            FT_STRING, BASE_NONE, nullptr, 0x0,
                                            "Protocols carried by this frame")},

        { hf_frame_color_filter_name,
            make_shared<QPAHeaderFieldInfo>("Coloring Rule Name", "frame.coloring_rule.name",
                                            FT_STRING, BASE_NONE, nullptr, 0x0,
                                            "The frame matched the coloring rule with this name")},

        { hf_frame_color_filter_text,
            make_shared<QPAHeaderFieldInfo>("Coloring Rule String", "frame.coloring_rule.string",
                                            FT_STRING, BASE_NONE, nullptr, 0x0,
                                            "The frame matched this coloring rule string")},

        { hf_frame_interface_id,
            make_shared<QPAHeaderFieldInfo>("Interface id", "frame.interface_id",
                                            FT_UINT32, BASE_DEC, nullptr, 0x0,
                                            "")},

        { hf_frame_pack_flags,
            make_shared<QPAHeaderFieldInfo>("Packet flags", "frame.packet_flags",
                                            FT_UINT32, BASE_HEX, nullptr, 0x0,
                                            "")},

        { hf_frame_pack_direction,
            make_shared<QPAHeaderFieldInfo>("Direction", "frame.packet_flags_direction",
                                            FT_UINT32, BASE_HEX, nullptr, PACKET_WORD_DIRECTION_MASK,
                                            "")},

        { hf_frame_pack_reception_type,
            make_shared<QPAHeaderFieldInfo>("Reception type", "frame.packet_flags_reception_type",
                                            FT_UINT32, BASE_DEC, nullptr, PACKET_WORD_RECEPTION_TYPE_MASK,
                                            "")},

        { hf_frame_pack_fcs_length,
            make_shared<QPAHeaderFieldInfo>("FCS length", "frame.packet_flags_fcs_length",
                                            FT_UINT32, BASE_DEC, nullptr, PACKET_WORD_FCS_LENGTH_MASK,
                                            "")},

        { hf_frame_pack_reserved,
            make_shared<QPAHeaderFieldInfo>("Reserved", "frame.packet_flags_reserved",
                                            FT_UINT32, BASE_DEC, nullptr, PACKET_WORD_RESERVED_MASK,
                                            "")},

        { hf_frame_pack_crc_error,
            make_shared<QPAHeaderFieldInfo>("CRC error", "frame.packet_flags_crc_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_CRC_ERR_MASK,
                                            "")},

        { hf_frame_pack_wrong_packet_too_long_error,
            make_shared<QPAHeaderFieldInfo>("Packet too long error", "frame.packet_flags_packet_too_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_PACKET_TOO_LONG_ERR_MASK,
                                            "")},

        { hf_frame_pack_wrong_packet_too_short_error,
            make_shared<QPAHeaderFieldInfo>("Packet too short error", "frame.packet_flags_packet_too_short_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_PACKET_TOO_SHORT_ERR_MASK,
                                            "")},

        { hf_frame_pack_wrong_inter_frame_gap_error,
            make_shared<QPAHeaderFieldInfo>("Wrong interframe gap error", "frame.packet_flags_wrong_inter_frame_gap_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_WRONG_INTER_FRAME_GAP_ERR_MASK,
                                            "")},

        { hf_frame_pack_unaligned_frame_error,
            make_shared<QPAHeaderFieldInfo>("Unaligned frame error", "frame.packet_flags_unaligned_frame_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_UNALIGNED_FRAME_ERR_MASK,
                                            "")},

        { hf_frame_pack_start_frame_delimiter_error,
            make_shared<QPAHeaderFieldInfo>("Start frame delimiter error", "frame.packet_flags_start_frame_delimiter_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_START_FRAME_DELIMITER_ERR_MASK,
                                            "")},

        { hf_frame_pack_preamble_error,
            make_shared<QPAHeaderFieldInfo>("Preamble error", "frame.packet_flags_preamble_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_PREAMBLE_ERR_MASK,
                                            "")},

        { hf_frame_pack_symbol_error,
            make_shared<QPAHeaderFieldInfo>("Symbol error", "frame.packet_flags_symbol_error",
                                            FT_BOOLEAN, 32, nullptr, PACKET_WORD_SYMBOL_ERR_MASK,
                                            "")},

        { hf_comments_text,
            make_shared<QPAHeaderFieldInfo>("Comment", "frame.comment",
                                            FT_STRING, BASE_NONE, nullptr, 0x0,
                                            "")}

    };

    QPAHeaderFieldRegisterInfo hf_encap ={
        hf_frame_wtap_encap,
        make_shared<QPAHeaderFieldInfo>("Encapsulation type", "frame.encap_type",
                                        FT_INT16, BASE_DEC, nullptr, 0x0,
                                        "")
    };

    QPAProtocolHelper *pHelper = QPAProtocolHelper::instance();

    proto_frame = pHelper->proto_register_protocol("Frame", "Frame", "frame");

    pHelper->proto_register_field_array(proto_frame, hf, sizeof (hf) / sizeof (hf[0]));
    ...
    helper->registerDissector("frame", dissect_frame, proto_frame);
