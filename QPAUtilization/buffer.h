...
class QPAUTILIZATIONSHARED_EXPORT Buffer
{
public:
    Buffer();
    ~Buffer();
    void ws_buffer_init(quint32 space = 1500);
    void ws_buffer_free();
    void ws_buffer_assure_space(quint32 space);
    void ws_buffer_append(quint8 *from, quint32 bytes);
    void ws_buffer_remove_start(quint32 bytes);
    void ws_buffer_clean();
    void ws_buffer_increase_length(unsigned int bytes);
    quint32 ws_buffer_length();
    quint8* ws_buffer_start_ptr();
    quint8* ws_buffer_end_ptr();
    void ws_buffer_append_buffer(Buffer *src);
    quint8 *dataPtr() const;
private:
    quint8	*data;
    quint32	allocated;
    quint32	start;
