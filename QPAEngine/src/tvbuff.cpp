			     ...
			     const gint length, const guint encoding)
{
	...
	case ENC_UCS_4:
		strptr = (guchar*)tvb_get_ucs_4_string(scope, tvb, offset, length,
		    encoding & ENC_LITTLE_ENDIAN);
		break;
	...
	case ENC_3GPP_TS_23_038_7BITS:
		{
			...
			strptr = (guchar*)tvb_get_ts_23_038_7bits_string(scope, tvb, bit_offset, no_of_chars);
		}
		break;
	...
	case ENC_ASCII_7BITS:
		{
			gint bit_offset  = offset << 3;
			gint no_of_chars = (length << 3) / 7;
			strptr = (guchar*)tvb_get_ascii_7bits_string(scope, tvb, bit_offset, no_of_chars);
		}
		break;
